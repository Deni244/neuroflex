document.addEventListener('DOMContentLoaded', () => {
  ...
  if (titleElement && slides[currentIndex]) {
    const slideTextElement = slides[currentIndex].querySelector('p');
    const newTitle = slideTextElement?.textContent?.trim();
    ...
    if (newTitle && newTitle !== '') {
      ...
      titleElement.textContent = `Lörem ipsum dorade boktig till geosylig postmodern ${currentIndex + 1}`;
    }
  }
};
